...
def main():
    ...
    C, R, V, F = market['C'], market['R'], market['V'], market['F']
    idx = market['index']
    ...
    jit(prepped[0][1], C, R, V, prepped[0][2], F, VOL, EXEC, INERTIA, ANN, LAMBDA, _scratch)

    worst_corr, worst_diff = 1.0, 0.0
    t_py = t_jit = 0.0
    for name, A, E in prepped:
        t0 = time.perf_counter(); cap_py = _sim_kernel_impl(A, C, R, V, E, F, VOL, EXEC, INERTIA, ANN, LAMBDA, _scratch.copy())
        t1 = time.perf_counter(); cap_jit = jit(A, C, R, V, E, F, VOL, EXEC, INERTIA, ANN, LAMBDA, _scratch.copy())
